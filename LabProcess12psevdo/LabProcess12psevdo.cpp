...
int main() {
    ...
    HANDLE hpsFile = GetCurrentProcess();

    if (!DuplicateHandle(GetCurrentProcess(), hFile, GetCurrentProcess(), &hpsFile,
        0, TRUE, DUPLICATE_SAME_ACCESS)) {
        std::cerr << "Ошибка дублирования дескриптора: " << GetLastError() << std::endl;
        CloseHandle(hFile);
        return 1;
    }

    wchar_t cmdline[256];
    swprintf_s(cmdline, L"C:\\Users\\st310-11\\Desktop\\меньшиков ПР-32\\LabProcess-main\\LabProcess12child\\x64\\Debug\\LabProcess12child.exe %d", (int)hpsFile);
